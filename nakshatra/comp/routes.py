...
def treasure(question_id):
    ...
    previous_submission = Treasuresubmission.query.filter_by(user_id=current_user.id,question_id=question_id-1).first()
    if previous_submission is None and question_id-1>=1:
        return redirect(url_for('comp.treasure',question_id=question_id-1))
    if request.method =='POST':
        if current_question.answer != request.form.get('answer'):
            flash(f'Wrong Answer. All answer should be in small letters. Attempt count: {current_user.count+1}','warning')
            current_user.count=current_user.count+1
            db.session.commit()
            return render_template('comp/treasure.html', question=current_question, title='Treasure Hunt')
...
@comp.route('/treasureHunt')
@login_required
def treasure_rule():
    return render_template('comp/treasure_rule.html', title='Treasure Hunt')